    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
            ...
            <div className="flex items-center justify-between mb-8">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors group px-4 py-2 rounded-lg hover:bg-zinc-800"
                >
            ...
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 sm:p-10 shadow-2xl relative overflow-hidden min-h-[500px]"
                >
                    {/* Subtle Background Glow */}
                    <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-emerald-500/5 rounded-full blur-[100px] pointer-events-none" />

                    <div className="relative z-10 space-y-4 mb-12 border-b border-zinc-800 pb-8">
                        <div className="flex justify-between items-center">
                            <div className="text-emerald-400 font-bold tracking-widest uppercase text-sm flex items-center gap-2">
                                {chapter.title}
                            </div>
                            <div className="text-zinc-500 text-xs font-bold uppercase tracking-widest bg-zinc-800/50 px-3 py-1 rounded-full border border-zinc-700/50">
                    ...
                    <div className="relative z-10 space-y-8">
                        {currentPage.map((block, idx) => {
                            if (block.type === 'paragraph') {
                                return (
                                    <div key={idx} className="space-y-2">
                                        ...
                                        <p className="text-lg leading-relaxed text-zinc-300 font-medium tracking-wide">
                            ...
                            if (block.type === 'quote') {
                                return (
                                    <blockquote key={idx} className="my-10 px-8 py-8 border-l-4 border-emerald-500 bg-emerald-500/5 rounded-r-2xl border-y border-r border-zinc-800">
                            ...
                            if (block.type === 'list') {
                                return (
                                    <div key={idx} className="my-8">
                                        {block.title && <h3 className="text-xl font-bold text-white mb-4">{block.title}</h3>}
                            ...
                            if (block.type === 'image') {
                                return (
                                    <figure key={idx} className="my-12 flex flex-col items-center">
                                        <div className="relative overflow-hidden rounded-2xl border border-zinc-700 shadow-xl">
                                            <img
                                        ...
                                        {block.caption && (
                                            <figcaption className="mt-4 text-center text-sm font-medium text-zinc-500">
                                                {block.caption}
                                            </figcaption>
                                        )}
                                    </figure>
                    ...
                    <div className="relative z-10 mt-12 pt-8 border-t border-zinc-800 flex justify-between items-center">
                        <button
                        ...
                        <div className="flex gap-1">
                            {pages.map((_, i) => (
                                <div
                                    key={i}
                                    className={`w-2 h-2 rounded-full transition-all duration-300 ${i === currentPageIndex ? 'bg-emerald-500 w-8' : 'bg-zinc-700'
                                        }`}
                                />
                            ))}
                        </div>
                        ...
                        {!isLastPage && (
                            <button
                                ...
                                className="flex items-center gap-2 px-8 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-400 transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)]"
                            >
                                Continue
            ...
            <AnimatePresence>
                {isLastPage && (
                    <motion.div
                        ...
                        className="mt-12 bg-zinc-900 border border-zinc-800 rounded-3xl p-10 shadow-2xl text-center relative overflow-hidden"
                    >
                        ...
                        <p className="text-zinc-400 mb-8">Answer questions correctly to earn up to <span className="text-emerald-400 font-bold">100 XP</span>.</p>
                        <button
                            onClick={onStartQuiz}
                            className="inline-flex items-center gap-3 px-8 py-4 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-400 transition-all hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(16,185,129,0.3)] min-w-[200px] justify-center text-lg"
                        >
                            <PlayCircle className="w-5 h-5" />
                            Start Quiz
                        </button>
...
const ActionBlock = ({ block }: { block: ContentBlock }) => {
    return (
        <div className="my-12 bg-zinc-950 border-l-4 border-emerald-500 p-8 rounded-r-2xl shadow-lg relative overflow-hidden group">
    ...
    return (
        <div className="my-8">
            <button
                ...
                className="w-full group flex items-center justify-between p-6 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 transition-all duration-300 text-left"
            >
                <div className="flex items-center gap-4">
                    <div className="flex shrink-0 items-center justify-center w-12 h-12 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 group-hover:bg-emerald-500/10 group-hover:text-emerald-400 group-hover:border-emerald-500/20 transition-all">
                ...
                <div className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 group-hover:bg-zinc-800 transition-colors">
            ...
            <AnimatePresence>
                {isExpanded && (
                    ...
                    >
                        <div className="p-6 mt-4 rounded-2xl bg-zinc-950 border border-zinc-800/50 shadow-inner">
                            {Array.isArray(block.content) ? (
                                <div className="space-y-4">
                                    {block.content.map((p, i) => (
                                        <p key={i} className="text-zinc-400 leading-relaxed font-medium">
